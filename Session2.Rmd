...
library(R.utils)
library(here)
file_url <- "https://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/DVS/natality/Nat2021us.zip"

options(timeout=400)
download.file(url = file_url, 
              destfile = "Data/nat2021us.zip",
              mode = "wb",
              overwrite = TRUE)

# now unzip it (it's ~5Gb!)
# we use here() because this function apparently wants absolute file paths
unzip(zipfile = here("Data/nat2021us.zip"),
             exdir = here("Data"), 
             overwrite = TRUE,
...
gzip("Data/nat2021us.txt")

# and go ahead and delete the uncompressed text file. 
# You could also delete the zip version..
unlink("Data/nat2021us.txt")
```
...
Now we've redistributed births of unknown time of day according to the distribution of known time of day within each mother age. The *within* part of this was dealt with by the `group_by(mage)` statement, where one could add another variable or two to make it finer. However, already for mother age under ca 15 or over ca 35, we're not filling each minute of the day with observations. Were we to zoom in we might even observe minute heaping...
...
Do you find this data too noisy when plotted by single minutes? Try grouping to 5-minute bins. Recall the modulo trick (`%%`).

Or you could aggregate for a few years and tell me whether the distribution has changed over time. Or during the pandemic? 

Or you could look at the birth weight distribution by .... You get the idea, there are tons of exremely interesting things you could explore and plot in this data, any of which would suffice for an assignment.
